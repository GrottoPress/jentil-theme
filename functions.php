...
if (version_compare(PHP_VERSION, MINIMUM_PHP, '<') ||
    ...
    add_action('admin_notices', 'jentilThemeRequiredNotice');

    jentilThemeDeactivate();
} else {
...
function jentilThemeRequiredNotice()
{
...
function jentilThemeDeactivate()
{
    $themes = wp_get_themes(['allowed' => true]);
    unset($themes[THEME_SLUG], $themes['jentil']);
